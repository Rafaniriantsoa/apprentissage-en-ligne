// src/components/etudiant/ProfilEtudiant.jsx

import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { User, Mail, Lock, CheckCircle, AlertTriangle, Clock, Edit, Camera, X } from 'lucide-react';

// --- Configuration API ---
const LIRE_PROFIL_API = 'http://localhost/projet-plateforme/backend/api/etudiant/lireProfil.php';
const MODIFIER_PROFIL_API = 'http://localhost/projet-plateforme/backend/api/etudiant/modifierProfil.php';
// URL de base pour afficher l'image
...
const ProfilEtudiant = () => {
    ...
    const [formData, setFormData] = useState({
        nom_complet: '',
        email: '',
        photo: '', // Chemin actuel de la photo dans la DB
        mot_de_passe: '',
    ...
    // --- 1. Récupération des données initiales (Pré-remplissage) ---
    useEffect(() => {
        const fetchProfil = async () => {
            if (!idEtudiant) {
                setError("ID Étudiant manquant. Veuillez vous reconnecter.");
                setLoading(false);
                return;
            }

            try {
                const response = await axios.get(`${LIRE_PROFIL_API}?id_etudiant=${idEtudiant}`);
                
                if (response.data.success) {
                    // Utilise 'etudiant' comme clé (selon lireProfil.php)
                    const data = response.data.etudiant; 
    ...
    // --- Gestion des changements de formulaire (texte) ---
    ...
    // --- Gestion de la sélection du fichier et de l'aperçu ---
    ...
    const handleSubmit = async (e) => {
        ...
        // Le nom de clé 'id_etudiant' doit correspondre au $_POST['id_etudiant'] dans PHP
        dataToSend.append('id_etudiant', idEtudiant); 
        dataToSend.append('nom_complet', formData.nom_complet);
        dataToSend.append('email', formData.email);
        // Pas de spécialité pour les étudiants

        if (fileToUpload) {
            // Clé 'photo_file' correspond à $_FILES['photo_file'] dans PHP
    ...
    const currentPhotoUrl = filePreview 
        ...
        : (formData.photo && !imageError) 
            ? `${BASE_URL_PHOTO}${formData.photo}` // Utilise BASE_URL_PHOTO + chemin DB
            : null;
    ...
    const placeholderContent = fileToUpload ? (
        ...
        <p className='text-xs text-center mt-1 text-red-600'>Image introuvable</p>
    ) : (
    ...
    return (
        <div className="max-w-3xl mx-auto p-6 md:p-10 bg-white shadow-2xl rounded-xl mt-10">
            <h1 className="text-3xl font-bold text-indigo-700 mb-6 flex items-center border-b pb-3">
                <User className="w-7 h-7 mr-3" /> Mon Profil Étudiant
            </h1>
